        ...
        <div class="m-4 text-center">
            ...
            <div class="py-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                @foreach ($featured_apps as $app)
                    <div class="block @if ($app->_compiled->image_type == 'cover' && $app->_compiled->image_src != null) aspect-video @endif sm:aspect-video overflow-hidden bg-white dark:bg-gray-800 rounded-xl shadow-lg text-lg">
                        @if ($app->_compiled->image_type == 'cover' && $app->_compiled->image_src != null)
                            <picture>
                                <img alt="Cover image for {{ $app->name }}" src="{{ $app->_compiled->image_src }}" height="1080" width="1920" class="h-full w-full aspect-video bg-no-repeat object-cover" />
                            </picture>
                        @endif
